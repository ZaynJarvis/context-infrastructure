...
This repo ships the AI heartbeat jobs. Other personal automation jobs from the
upstream author are not part of this starter contract.
...
Optional  → AI News Survey: 生成 AI 行业日报或周报
```
...
2. **虚拟环境**：脚本依赖 `.venv` 中的 Python 包，确保先运行 `pip install -r requirements.txt`。
